...
export default function Vr_display(props) {
  ...
  const nextSlide = () => {
    setSlide(slide === props.data.picture.length - 1 ? 0 : slide + 1);
  };

  const prevSlide = () => {
    setSlide(slide === 0 ? props.data.picture.length - 1 : slide - 1);
  };
          ...
          <div class="card1 dontshow1 imagecontainer1 " id="image1">
            ...
            <div class="label1">
              <div className="carousel1">
                <div className="arrow1 arrow-left1" onClick={prevSlide}>
                  ❰
                </div>
                {props.data.picture?.map((item, idx) => {
                  return (
                    <img
                      src={item}
                      alt="img"
                      key={idx}
                      className={slide === idx ? "slide1" : "slide1 slide-hidden1"}
                    />
                  );
                })}

                <div className="arrow1 arrow-right1" onClick={nextSlide}>
                  ❱
                </div>
                <span className="indicators1">
                  {props.data.picture?.map((_, idx) => {
                    return (
                      <button
                        key={idx}
                        className={
                          slide === idx ? "indicator1" : "indicator1 indicator-inactive1"
                        }
